...
3. **Selector generation and dual-locator re-resolution** (Layers 1 and 3). Directly drops into YoinkIt's `on(sel)` / `pick()`. See [report 04](reports/04-live-mode-manual-edits.md) and [report 01](reports/01-detector-engine/01-detector-engine.md).
