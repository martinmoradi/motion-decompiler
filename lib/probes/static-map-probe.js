(function () {
  ...
  function assetsFor(el) {
    var assets = [];
    var seen = {};
    function pushAsset(asset, kind, url, extra) {
      var resolved = absUrl(url);
      var key = selectorFor(asset) + ':' + kind + ':' + resolved;
      if (!resolved || seen[key]) return;
      seen[key] = true;
    ...
    var nodes = [el].concat(Array.prototype.slice.call(el.querySelectorAll('*')).filter(visible));
    nodes.slice(0, 250).forEach(function (asset) {
      var cs = getComputedStyle(asset);
      var tag = asset.tagName.toLowerCase();
      if (tag === 'img' || tag === 'video' || tag === 'source') {
        pushAsset(asset, tag, asset.currentSrc || asset.src || asset.getAttribute('src') || asset.getAttribute('srcset'));
      }
      urlsFromCss(cs.backgroundImage && cs.backgroundImage !== 'none' ? cs.backgroundImage : '').forEach(function (url) {
        pushAsset(asset, 'background-image', url, { evidence: 'computed-style.backgroundImage' });
      });
    });
