...
const DEFAULT_CROP_SETTLE_MS = 500;
...
function applyCropEvidence({ facts, selector, outputFile, relativePath, result }) {
  ...
  facts.crop = {
    ...
    bytes,
    selector,
    method: (result && result.method) || 'browser-viewport-screenshot',
  };
}
...
function topAlignSelectorScript(selector) {
  return `(() => {
    const selector = ${JSON.stringify(selector)};
    const el = document.querySelector(selector);
    if (!el) throw new Error(\`Static Map crop selector did not match: \${selector}\`);
    const before = el.getBoundingClientRect();
    const top = Math.max(0, before.top + window.scrollY);
    const left = Math.max(0, before.left + window.scrollX);
...
function createCaptureBrowserStaticMapDriver(options = {}) {
  ...
  const cropSettleMs = options.cropSettleMs == null ? DEFAULT_CROP_SETTLE_MS : Number(options.cropSettleMs);
  ...
  function prepareCrop(selector) {
    run(['eval', topAlignSelectorScript(selector), '--max-output', '500000']);
    if (cropSettleMs > 0) sleepMs(cropSettleMs);
  }

  function viewportCropResult(viewport) {
    return {
  ...
  return {
    ...
    captureRegionCrop({ targetUrl, viewport, selector, outputFile }) {
      ...
      run(['set', 'viewport', String(viewport.width), String(viewport.height)]);
      if (openDelayMs > 0) sleepMs(openDelayMs);
      prepareCrop(selector);
      run(['screenshot', outputFile]);
      return viewportCropResult(viewport);
    },
    captureRegionCrops({ targetUrl, viewport, crops }) {
      ...
      return (crops || []).map((crop) => {
        prepareCrop(crop.selector);
        run(['screenshot', crop.outputFile]);
        return viewportCropResult(viewport);
      });
    },
  };
