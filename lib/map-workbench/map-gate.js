...
function approvedScopeMatchesRegion(regionId, approvedExceptionRecords) {
  return Boolean(approvedExceptionForRegion(regionId, approvedExceptionRecords));
}
...
function assertionWaiverRecord(row, pageModel, approvedExceptions, approvedExceptionRecords) {
  ...
  if (exceptionId && approvedExceptions.has(exceptionId)) {
    ...
    return approvedScopeMatchesRegion(regionId, approvedRecord ? [approvedRecord] : []) ? approvedRecord : null;
  }
...
function coverageExcepted(row, pageModel, approvedExceptionRecords) {
  return Boolean(coverageWaiverRecord(row, pageModel, approvedExceptionRecords));
}
...
function coverageBlockers(markdown, source, pageModel = {}, approvedExceptionRecords = [], waiverAudit = null) {
  const blockers = [];
  for (const row of parseCoverageRows(markdown)) {
...
function reportInputIsFresh(freshness, inputId) {
  return !freshness.blockers.some(item => item && item.source === 'report-freshness' && item.id === inputId);
}

function rebaselineReportSnapshotPageModel(snapshotFile, reportFile, snapshot, pageModel, pageModelHash) {
  snapshot.pageModel = pageModel;
  if (!snapshot.inputHashes || typeof snapshot.inputHashes !== 'object') snapshot.inputHashes = {};
  snapshot.inputHashes['page-model.json'] = pageModelHash;
  writeJson(snapshotFile, snapshot);
  const reportStat = fs.statSync(reportFile);
  const snapshotStat = fs.statSync(snapshotFile);
  const freshReportMtime = new Date(snapshotStat.mtimeMs + 1000);
  fs.utimesSync(reportFile, reportStat.atime, freshReportMtime);
}
...
function runMapGate(runDir, decision, env = {}) {
  ...
  const freshness = applyReportArtifactFreshness(absRunDir, evaluateFreshness(absRunDir, snapshot), reportFile, snapshotFile);
  ...
  if (decision.action === 'approve-exception') {
    const pageModelWasFresh = reportInputIsFresh(freshness, 'page-model.json');
    ...
    const pageModelHash = sha256File(path.join(absRunDir, 'page-model.json'));
    freshness.hashes['page-model.json'] = pageModelHash;
    if (pageModelWasFresh) {
      rebaselineReportSnapshotPageModel(snapshotFile, reportFile, snapshot, pageModel, pageModelHash);
    }
  }
