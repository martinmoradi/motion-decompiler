'use strict';

const crypto = require('crypto');
const fs = require('fs');
const path = require('path');
const {
  configPath,
  loadRunConfig,
  loadPageModel,
  mapReportDir,
  motionScoutDir,
  readJson,
  savePageModel,
  staticMapDir,
  writeJson,
} = require('./artifacts');
const { mapWorkbenchRequiredInputFiles } = require('./map-inputs');
const { REQUIRED_DISCOVERY_SOURCES } = require('./motion-scout');

const MAP_GATE_SCHEMA_VERSION = 1;
const REQUIRED_INPUT_SCHEMA_VERSION = 1;

class MapGateBlockedError extends Error {
  constructor(message, gate) {
    super(message);
    this.name = 'MapGateBlockedError';
    this.gate = gate;
  }
}

function nowIso(env = {}) {
  const now = env.now ? new Date(env.now) : new Date();
  return now.toISOString();
}

function runRelativePath(runDir, file) {
  return path.relative(runDir, file).split(path.sep).join('/');
}

function sha256File(file) {
  return crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex');
}

function requiredInputHashKeys(runDir) {
  return mapWorkbenchRequiredInputFiles(runDir).map(file => runRelativePath(runDir, file));
}

function containedRunPath(runDir, relativeFile) {
  const root = path.resolve(runDir);
  const resolved = path.resolve(root, relativeFile);
  if (resolved === root || resolved.startsWith(`${root}${path.sep}`)) return resolved;
  return null;
}

function evaluateFreshness(runDir, snapshot) {
  const expected = snapshot && snapshot.inputHashes && typeof snapshot.inputHashes === 'object'
    ? snapshot.inputHashes
    : null;
  const hashes = {};
  const blockers = [];
  const requiredKeys = requiredInputHashKeys(runDir);
  const requiredKeySet = new Set(requiredKeys);

  if (!expected || Object.keys(expected).length === 0) {
    blockers.push({
      id: 'report-snapshot-input-hashes',
      source: 'report-freshness',
      status: 'missing',
      message: 'Report v0 input hashes are missing; rerun map-report',
    });
    return {
      hashes,
      blockers,
      summary: {
        staleInputs: 0,
        missingInputs: 1,
        unexpectedInputs: 0,
      },
    };
  }

  for (const relativeFile of Object.keys(expected)) {
    const file = containedRunPath(runDir, relativeFile);
    if (!file) {
      blockers.push({
        id: relativeFile,
        source: 'report-freshness',
        status: 'invalid',
        message: `${relativeFile} escapes the run directory; rerun map-report`,
      });
      continue;
    }
    if (!requiredKeySet.has(relativeFile)) {
      blockers.push({
        id: relativeFile,
        source: 'report-freshness',
        status: 'unexpected',
        message: `${relativeFile} is not a Map Report input; rerun map-report`,
      });
    }
  }

  for (const relativeFile of requiredKeys) {
    if (!Object.prototype.hasOwnProperty.call(expected, relativeFile)) {
      blockers.push({
        id: relativeFile,
        source: 'report-freshness',
        status: 'missing',
        message: `${relativeFile} is missing from Report v0 input hashes; rerun map-report`,
      });
      continue;
    }
    const expectedHash = expected[relativeFile];
    const file = containedRunPath(runDir, relativeFile);
    if (!fs.existsSync(file)) {
      blockers.push({
        id: relativeFile,
        source: 'report-freshness',
        status: 'missing',
        message: `${relativeFile} is missing; rerun map-report`,
      });
      continue;
    }
    const hash = sha256File(file);
    hashes[relativeFile] = hash;
    if (hash !== expectedHash) {
      blockers.push({
        id: relativeFile,
        source: 'report-freshness',
        status: 'stale',
        message: `${relativeFile} changed after Report v0; rerun map-report`,
      });
    }
  }

  return {
    hashes,
    blockers,
    summary: {
      staleInputs: blockers.filter(item => item.status === 'stale').length,
      missingInputs: blockers.filter(item => item.status === 'missing').length,
      unexpectedInputs: blockers.filter(item => item.status === 'unexpected' || item.status === 'invalid').length,
    },
  };
}

function applyReportArtifactFreshness(runDir, freshness, reportFile, snapshotFile) {
  const reportStat = fs.statSync(reportFile);
  const snapshotStat = fs.statSync(snapshotFile);
  if (reportStat.mtimeMs >= snapshotStat.mtimeMs) return freshness;

  const id = runRelativePath(runDir, reportFile);
  freshness.blockers.push({
    id,
    source: 'report-freshness',
    status: 'stale',
    message: `${id} is older than 04-map-report/report-snapshot.json; rerun map-report`,
  });
  freshness.summary.staleInputs = (freshness.summary.staleInputs || 0) + 1;
  return freshness;
}

function exceptionIdFor(row) {
  return row && (row.exceptionId || null);
}

function pageRegions(pageModel) {
  const home = pageModel && pageModel.pages && pageModel.pages.home;
  return home && Array.isArray(home.regions) ? home.regions : [];
}

function knownRegionIds(pageModel) {
  return pageRegions(pageModel)
    .map(region => region && region.id)
    .filter(Boolean);
}

function knownGeneratedTargetSuffixes(pageModel) {
  const viewports = Array.isArray(pageModel && pageModel.viewports) ? pageModel.viewports : [];
  const viewportCropSuffixes = viewports
    .map(viewport => viewport && viewport.id)
    .filter(Boolean)
    .map(viewportId => `${viewportId}-crop`);
  return new Set([
    'geometry',
    'placeholder',
    'crop',
    'assets',
    'typography',
    'unknowns',
    'evidence-completeness',
    'attachment',
    ...viewportCropSuffixes,
  ]);
}

function generatedTargetMatchesRegion(target, prefix, regionId, pageModel) {
  const marker = `${prefix}-${regionId}-`;
  if (!target.startsWith(marker)) return false;
  return knownGeneratedTargetSuffixes(pageModel).has(target.slice(marker.length));
}

function regionIdForId(id, pageModel) {
  const target = String(id || '');
  return knownRegionIds(pageModel)
    .sort((left, right) => right.length - left.length)
    .find(regionId => (
      generatedTargetMatchesRegion(target, 'static-map', regionId, pageModel) ||
      generatedTargetMatchesRegion(target, 'motion-scout', regionId, pageModel)
    )) || null;
}

function assertionRegionId(row, pageModel) {
  if (row && row.regionId && knownRegionIds(pageModel).includes(row.regionId)) return row.regionId;
  return regionIdForId(row && row.id, pageModel);
}

function approvedExceptionForRegion(regionId, approvedExceptionRecords) {
  if (!regionId) return false;
  return approvedExceptionRecords.find(item => (
    item &&
    item.scope &&
    item.scope.kind === 'region' &&
    item.scope.id === regionId
  )) || null;
}

function assertionWaiverRecord(row, pageModel, approvedExceptions, approvedExceptionRecords) {
  const regionId = assertionRegionId(row, pageModel);
  const exceptionId = exceptionIdFor(row);
  if (exceptionId && approvedExceptions.has(exceptionId)) {
    const approvedRecord = approvedExceptionRecords.find(item => item && item.id === exceptionId);
    return approvedExceptionForRegion(regionId, approvedRecord ? [approvedRecord] : []) ? approvedRecord : null;
  }
  return approvedExceptionForRegion(regionId, approvedExceptionRecords);
}

function assertionExcepted(row, pageModel, approvedExceptions, approvedExceptionRecords) {
  return Boolean(assertionWaiverRecord(row, pageModel, approvedExceptions, approvedExceptionRecords));
}

function requiredRowPasses(row, passingStatus) {
  return String(row && row.status || '').toLowerCase().trim() === passingStatus;
}

function recordExceptionWaiver(waiverAudit, exceptionRecord, blockerId) {
  if (!waiverAudit || !exceptionRecord || !(typeof exceptionRecord.id === 'string' && exceptionRecord.id.trim()) || !blockerId) return;
  if (!waiverAudit.waivedById.has(exceptionRecord.id)) waiverAudit.waivedById.set(exceptionRecord.id, new Set());
  waiverAudit.waivedById.get(exceptionRecord.id).add(blockerId);
}

function createWaiverAudit(approvedExceptionRecords) {
  return {
    records: approvedExceptionRecords,
    waivedById: new Map(approvedExceptionRecords.map(record => [record.id, new Set()])),
  };
}

function exceptionWaivers(waiverAudit) {
  if (!waiverAudit) return [];
  return waiverAudit.records.map(record => ({
    id: record.id,
    scope: record.scope,
    waived: Array.from(waiverAudit.waivedById.get(record.id) || []),
  }));
}

function requiredFailures(assertionArtifact, source, approvedExceptions = new Set(), pageModel = {}, approvedExceptionRecords = [], waiverAudit = null) {
  const rows = Array.isArray(assertionArtifact && assertionArtifact.assertions)
    ? assertionArtifact.assertions
    : [];
  const blockers = [];
  for (const row of rows) {
    if (!(row && row.required && !requiredRowPasses(row, 'pass'))) continue;
    const waiver = assertionWaiverRecord(row, pageModel, approvedExceptions, approvedExceptionRecords);
    if (waiver) {
      recordExceptionWaiver(waiverAudit, waiver, row.id);
      continue;
    }
    blockers.push({
      id: row.id,
      source,
      kind: row.kind || null,
      status: row.status || 'missing',
      message: row.failure || row.id,
      evidence: Array.isArray(row.evidence) ? row.evidence : [],
    });
  }
  return blockers;
}

function inputShapeBlocker(id, message) {
  return {
    id,
    source: 'report-input',
    status: 'invalid',
    message,
  };
}

function assertionInputBlockers(assertionArtifact, artifactId) {
  const blockers = [];
  const artifact = assertionArtifact && typeof assertionArtifact === 'object' && !Array.isArray(assertionArtifact)
    ? assertionArtifact
    : null;
  if (!artifact || artifact.schemaVersion !== REQUIRED_INPUT_SCHEMA_VERSION) {
    blockers.push(inputShapeBlocker(
      `${artifactId}:schema-version`,
      `${artifactId} has an invalid schemaVersion; rerun the producer and map-report`
    ));
  }
  if (!artifact || !Array.isArray(artifact.assertions)) {
    blockers.push(inputShapeBlocker(
      `${artifactId}:assertions`,
      `${artifactId} must contain an assertions array; rerun the producer and map-report`
    ));
  } else if (artifact.assertions.length === 0) {
    blockers.push(inputShapeBlocker(
      `${artifactId}:assertions`,
      `${artifactId} contains no assertion rows; rerun the producer and map-report`
    ));
  }
  return blockers;
}

function motionCandidatesInputBlockers(motionCandidatesArtifact) {
  const artifactId = '03-motion-scout/motion-candidates.json';
  const blockers = [];
  const artifact = motionCandidatesArtifact && typeof motionCandidatesArtifact === 'object' && !Array.isArray(motionCandidatesArtifact)
    ? motionCandidatesArtifact
    : null;
  if (!artifact || artifact.schemaVersion !== REQUIRED_INPUT_SCHEMA_VERSION) {
    blockers.push(inputShapeBlocker(
      `${artifactId}:schema-version`,
      `${artifactId} has an invalid schemaVersion; rerun motion-scout and map-report`
    ));
  }
  if (!artifact || !Array.isArray(artifact.candidates)) {
    blockers.push(inputShapeBlocker(
      `${artifactId}:candidates`,
      `${artifactId} must contain a candidates array; rerun motion-scout and map-report`
    ));
  }
  const discovery = artifact && artifact.discovery;
  if (!(discovery && typeof discovery === 'object' && !Array.isArray(discovery)) || !Array.isArray(discovery.inspections)) {
    blockers.push(inputShapeBlocker(
      `${artifactId}:discovery`,
      `${artifactId} must contain discovery.inspections; rerun motion-scout and map-report`
    ));
  }
  return blockers;
}

function gateInputBlockers(inputs = {}) {
  return []
    .concat(assertionInputBlockers(inputs.staticAssertions, '02-static-map/assertions.json'))
    .concat(assertionInputBlockers(inputs.motionAssertions, '03-motion-scout/assertions.json'))
    .concat(motionCandidatesInputBlockers(inputs.motionCandidates));
}

function assertionSummary(staticAssertions, motionAssertions, assertionBlockers = []) {
  const rows = []
    .concat(Array.isArray(staticAssertions.assertions) ? staticAssertions.assertions : [])
    .concat(Array.isArray(motionAssertions.assertions) ? motionAssertions.assertions : []);
  const required = rows.filter(row => row && row.required);
  const incompleteRequired = required.filter(row => !requiredRowPasses(row, 'pass')).length;
  const failedRequired = assertionBlockers.length;
  return {
    total: rows.length,
    required: required.length,
    failedRequired,
    exceptedRequired: Math.max(0, incompleteRequired - failedRequired),
  };
}

function tableCell(value) {
  return String(value || '').replace(/\\\|/g, '|').trim();
}

function headerCell(value) {
  return tableCell(value).toLowerCase().replace(/\s+/g, '-');
}

function cellAt(cells, headers, names) {
  for (const name of names) {
    const index = headers.indexOf(name);
    if (index !== -1) return cells[index] || '';
  }
  return '';
}

function splitTableCells(line) {
  const cells = [];
  let current = '';
  for (let index = 0; index < line.length; index += 1) {
    const char = line[index];
    if (char === '|' && line[index - 1] !== '\\') {
      cells.push(current);
      current = '';
      continue;
    }
    current += char;
  }
  cells.push(current);
  return cells;
}

function parseCoverageRows(markdown) {
  const rows = [];
  let headers = [];
  for (const rawLine of String(markdown || '').split(/\r?\n/)) {
    const line = rawLine.trim();
    if (!line.startsWith('|') || !line.endsWith('|')) {
      headers = [];
      continue;
    }
    if (/^\|\s*-/.test(line)) continue;
    const cells = splitTableCells(line.slice(1, -1)).map(tableCell);
    const normalized = cells.map(headerCell);
    if (normalized.includes('status')) {
      headers = normalized;
      continue;
    }
    if (!headers.includes('status')) continue;
    const area = cellAt(cells, headers, ['area', 'source', 'candidate', 'region']) || cells[0] || '';
    const required = cellAt(cells, headers, ['required']);
    const status = cellAt(cells, headers, ['status']);
    rows.push({
      area,
      name: cellAt(cells, headers, ['name', 'assertion', 'candidate']),
      required: /^(yes|true|required)$/i.test(required),
      status: String(status || '').toLowerCase(),
      evidence: cellAt(cells, headers, ['evidence']),
      reason: cellAt(cells, headers, ['reason']),
    });
  }
  return rows.filter(row => row.area);
}

function coveragePasses(row) {
  return requiredRowPasses(row, 'complete');
}

function coverageWaiverRecord(row, pageModel, approvedExceptionRecords) {
  const area = String(row && row.area || '').trim();
  const regionId = knownRegionIds(pageModel).includes(area)
    ? area
    : regionIdForId(row && row.name, pageModel);
  return approvedExceptionForRegion(regionId, approvedExceptionRecords);
}

function coverageBlockerId(row) {
  const name = String(row && row.name || '').trim();
  return /^(static-map|motion-scout)-/.test(name) ? name : row.area;
}

function coverageBlockers(markdown, source, pageModel = {}, approvedExceptionRecords = [], waiverAudit = null) {
  const blockers = [];
  const rows = parseCoverageRows(markdown);
  if (!rows.length) {
    return [{
      id: `${source}:unparseable`,
      source,
      status: 'invalid',
      message: `${source} did not contain a parseable coverage table; rerun the producer and map-report`,
      evidence: [],
    }];
  }
  for (const row of rows) {
    if (!(row.required && !coveragePasses(row))) continue;
    const waiver = coverageWaiverRecord(row, pageModel, approvedExceptionRecords);
    const id = coverageBlockerId(row);
    if (waiver) {
      recordExceptionWaiver(waiverAudit, waiver, id);
      continue;
    }
    blockers.push({
      id: coverageBlockerId(row),
      source,
      status: row.status || 'missing',
      message: row.reason || `${row.area} coverage is ${row.status || 'missing'}`,
      evidence: row.evidence ? [row.evidence] : [],
    });
  }
  return blockers;
}

function configuredViewportIds(config = {}, pageModel = {}) {
  const configIds = Array.isArray(config.viewports)
    ? config.viewports.map(viewport => viewport && viewport.id).filter(Boolean)
    : [];
  if (configIds.length) return configIds;
  return Array.isArray(pageModel.viewports)
    ? pageModel.viewports.map(viewport => viewport && viewport.id).filter(Boolean)
    : [];
}

function evidenceList(value) {
  if (Array.isArray(value)) return value.map(item => String(item || '').trim()).filter(Boolean);
  const text = String(value || '').trim();
  return text ? [text] : [];
}

function discoveryInspectionPasses(row) {
  // Only `complete` clears a required discovery row. A producer-authored
  // `out_of_scope` status is not a canonical waiver (CONTRACT.md Map Gate
  // rules): like static coverage, it still blocks until the source is actually
  // inspected. Discovery rows are page-global (source:viewport), so v0 has no
  // exception scope that can waive them — the source must be inspected.
  return String(row && row.status || '').toLowerCase().trim() === 'complete';
}

function motionDiscoveryBlockers(motionCandidatesArtifact, config = {}, pageModel = {}) {
  const discovery = motionCandidatesArtifact && motionCandidatesArtifact.discovery;
  const requiredSources = REQUIRED_DISCOVERY_SOURCES;
  const inspections = discovery && Array.isArray(discovery.inspections) ? discovery.inspections : [];
  const viewports = configuredViewportIds(config, pageModel);

  const bySourceViewport = new Map();
  for (const row of inspections) {
    const source = String(row && row.source || '').trim();
    const viewportId = String(row && row.viewportId || '').trim();
    if (!source || !viewportId) continue;
    bySourceViewport.set(`${source}\u0000${viewportId}`, row);
  }

  const blockers = [];
  if (!viewports.length) {
    blockers.push({
      id: 'motion-scout-discovery:viewports',
      source: 'motion-scout-discovery',
      status: 'missing',
      message: 'No configured viewports resolved for Motion Scout discovery coverage',
      evidence: [],
    });
  }
  for (const viewportId of viewports) {
    for (const source of requiredSources) {
      const row = bySourceViewport.get(`${source}\u0000${viewportId}`);
      if (row && discoveryInspectionPasses(row)) continue;
      blockers.push({
        id: `${source}:${viewportId}`,
        source: 'motion-scout-discovery',
        status: String(row && row.status || 'missing').toLowerCase().trim() || 'missing',
        message: row && row.reason ? row.reason : `${source} was not inspected for ${viewportId}`,
        evidence: row ? evidenceList(row.evidence) : [],
      });
    }
  }
  return blockers;
}

function motionScoutCoverageSummary(motionBlockers) {
  const blockers = Array.isArray(motionBlockers) ? motionBlockers : [];
  const viewportsResolved = !blockers.some(item => item && item.id === 'motion-scout-discovery:viewports');
  return {
    incompleteRequired: blockers.filter(item => item && item.id !== 'motion-scout-discovery:viewports').length,
    viewportsResolved,
  };
}

function coverageSummary(staticCoverageText, pageModel = {}, approvedExceptionRecords = [], motionDiscovery = [], waiverAudit = null) {
  const staticBlockers = coverageBlockers(staticCoverageText, 'static-map-coverage', pageModel, approvedExceptionRecords, waiverAudit);
  const motionBlockers = Array.isArray(motionDiscovery) ? motionDiscovery : [];
  return {
    summary: {
      staticMap: { incompleteRequired: staticBlockers.length },
      motionScout: motionScoutCoverageSummary(motionBlockers),
    },
    blockers: staticBlockers.concat(motionBlockers),
  };
}

function gateInputsFromSnapshot(snapshot) {
  return {
    staticAssertions: snapshot.staticMap && snapshot.staticMap.assertions,
    motionAssertions: snapshot.motionScout && snapshot.motionScout.assertions,
    staticCoverageText: snapshot.staticMap && snapshot.staticMap.coverageText,
    motionCandidates: snapshot.motionScout && snapshot.motionScout.candidates,
    pageModel: snapshot.pageModel,
  };
}

function readGateInputs(runDir, snapshot, freshness) {
  if (freshness.summary.missingInputs > 0) {
    return gateInputsFromSnapshot(snapshot);
  }
  return {
    staticAssertions: readJson(path.join(staticMapDir(runDir), 'assertions.json')),
    motionAssertions: readJson(path.join(motionScoutDir(runDir), 'assertions.json')),
    staticCoverageText: fs.readFileSync(path.join(staticMapDir(runDir), 'coverage.md'), 'utf8'),
    motionCandidates: readJson(path.join(motionScoutDir(runDir), 'motion-candidates.json')),
    pageModel: loadPageModel(runDir),
  };
}

function configFromSnapshot(snapshot = {}) {
  return {
    targetUrl: snapshot.targetUrl,
    primaryViewport: snapshot.primaryViewport,
    viewports: Array.isArray(snapshot.viewports) ? snapshot.viewports.slice() : [],
  };
}

function readGateConfig(runDir, snapshot, freshness) {
  if (fs.existsSync(configPath(runDir))) return loadRunConfig(runDir);
  if (freshness.summary.missingInputs > 0) return configFromSnapshot(snapshot);
  return loadRunConfig(runDir);
}

function blockedApprovalPrefix(freshness) {
  if (freshness.summary.missingInputs > 0) return 'Report v0 inputs are missing; rerun map-report';
  if (freshness.summary.staleInputs > 0) return 'Report v0 is stale; rerun map-report';
  return 'Map Gate approval blocked';
}

function unknownSummary(pageModel) {
  const unknowns = [];
  for (const region of pageRegions(pageModel)) {
    const rows = Array.isArray(region.unknowns) ? region.unknowns : [];
    for (const [index, unknown] of rows.entries()) {
      const field = unknown && unknown.field ? unknown.field : `unknown-${index + 1}`;
      unknowns.push({
        id: `${region.id}:${field}`,
        regionId: region.id,
        field,
        reason: unknown && typeof unknown.reason === 'string' ? unknown.reason.trim() : '',
      });
    }
  }
  const blockers = unknowns
    .filter(item => !item.reason)
    .map(item => ({
      id: item.id,
      source: 'page-model-unknowns',
      status: 'unknown',
      message: `${item.id} is unknown without a reason`,
    }));
  return {
    summary: {
      total: unknowns.length,
      withoutReason: blockers.length,
    },
    blockers,
  };
}

function parseScope(scope) {
  const match = String(scope || '').match(/^([^:]+):(.+)$/);
  if (!match || !match[1].trim() || !match[2].trim()) {
    throw new Error('map-gate --approve-exception requires --scope <kind:id>');
  }
  const kind = match[1].trim();
  if (kind !== 'region') {
    throw new Error(`map-gate --approve-exception only supports --scope region:<region-id> in v0; received unsupported scope kind "${kind}"`);
  }
  return { kind, id: match[2].trim() };
}

function approvedExceptionIds(pageModel) {
  return approvedExceptionRecords(pageModel).map(item => item.id);
}

function approvedExceptionValidationError(item, pageModel) {
  if (!(item && item.stage === 'map-gate' && item.approvedBy === 'human' && item.approvedAt)) {
    return null;
  }
  if (!(typeof item.id === 'string' && item.id.trim())) {
    return 'map-gate exception is missing a canonical id';
  }
  if (!(typeof item.reason === 'string' && item.reason.trim())) {
    return `${item.id} is missing a human-readable reason`;
  }
  if (!(item.scope && typeof item.scope === 'object' && !Array.isArray(item.scope))) {
    return `${item.id} scope must be a canonical object`;
  }
  if (item.scope.kind !== 'region') {
    return `${item.id} uses unsupported scope kind ${item.scope.kind || 'unknown'}`;
  }
  if (!(typeof item.scope.id === 'string' && knownRegionIds(pageModel).includes(item.scope.id))) {
    return `${item.id} scope region:${item.scope.id || 'unknown'} does not match a Page model Region`;
  }
  return null;
}

function isCanonicalApprovedException(item, pageModel) {
  return item &&
    item.stage === 'map-gate' &&
    item.approvedBy === 'human' &&
    item.approvedAt &&
    !approvedExceptionValidationError(item, pageModel);
}

function approvedExceptionRecords(pageModel) {
  return (Array.isArray(pageModel.exceptions) ? pageModel.exceptions : [])
    .filter(item => isCanonicalApprovedException(item, pageModel));
}

function duplicateMapGateExceptionIds(pageModel) {
  const counts = new Map();
  for (const item of (Array.isArray(pageModel.exceptions) ? pageModel.exceptions : [])) {
    if (!(item && item.stage === 'map-gate' && typeof item.id === 'string' && item.id.trim())) continue;
    const id = item.id.trim();
    counts.set(id, (counts.get(id) || 0) + 1);
  }
  return Array.from(counts.entries())
    .filter(([, count]) => count > 1)
    .map(([id, count]) => ({ id, count }));
}

function exceptionBlockers(pageModel) {
  const duplicateBlockers = duplicateMapGateExceptionIds(pageModel).map(({ id, count }) => ({
    id,
    source: 'page-model-exceptions',
    status: 'duplicate',
    message: `map-gate exception id "${id}" appears ${count} times; remove duplicate Page model exceptions before approving`,
  }));
  return duplicateBlockers.concat((Array.isArray(pageModel.exceptions) ? pageModel.exceptions : [])
    .filter(item => item && item.stage === 'map-gate')
    .map(item => ({
      item,
      validationError: approvedExceptionValidationError(item, pageModel),
    }))
    .filter(({ item, validationError }) => validationError || !isCanonicalApprovedException(item, pageModel))
    .map(item => ({
      id: (typeof item.item.id === 'string' && item.item.id.trim()) ? item.item.id : 'map-gate-exception:invalid',
      source: 'page-model-exceptions',
      status: 'exception',
      message: item.validationError || item.item.reason || `${item.item.id || 'map-gate exception'} requires human approval`,
    })));
}

function approveException(pageModel, decision, approvedAt) {
  if (!(typeof decision.reason === 'string' && decision.reason.trim())) {
    throw new Error('map-gate --approve-exception requires --reason with a human-readable reason');
  }
  const scope = parseScope(decision.scope);
  if (scope.kind === 'region' && !knownRegionIds(pageModel).includes(scope.id)) {
    throw new Error(`map-gate --approve-exception scope region:${scope.id} does not match a Page model Region`);
  }
  const next = JSON.parse(JSON.stringify(pageModel));
  if (!Array.isArray(next.exceptions)) next.exceptions = [];
  const existingRecords = next.exceptions.filter(item => item && item.id === decision.exceptionId);
  const crossStageRecord = existingRecords.find(item => item.stage !== 'map-gate');
  if (crossStageRecord) {
    throw new Error(`map-gate --approve-exception id "${decision.exceptionId}" already exists for stage ${crossStageRecord.stage || 'unknown'}; choose a map-gate exception id`);
  }
  const sameStageRecords = existingRecords.filter(item => item.stage === 'map-gate');
  if (sameStageRecords.length > 1) {
    throw new Error(`map-gate --approve-exception id "${decision.exceptionId}" already exists more than once for stage map-gate; remove duplicate Page model exceptions before approving`);
  }
  const existing = sameStageRecords[0];
  const record = existing || { id: decision.exceptionId };
  record.stage = 'map-gate';
  record.scope = scope;
  record.reason = decision.reason.trim();
  record.approvedBy = 'human';
  record.approvedAt = approvedAt;
  if (decision.expiresAfterStage) record.expiresAfterStage = decision.expiresAfterStage;
  if (!existing) next.exceptions.push(record);
  return next;
}

function writeGate(runDir, gate) {
  const file = path.join(mapReportDir(runDir), 'gate.json');
  writeJson(file, gate);
  return file;
}

function validateDecision(decision = {}) {
  if (!['approve', 'reject', 'approve-exception'].includes(decision.action)) {
    throw new Error('map-gate requires one of approve, reject, or approve-exception');
  }
  if (decision.action === 'reject' && !(typeof decision.reason === 'string' && decision.reason.trim())) {
    throw new Error('map-gate --reject requires --reason with a human-readable reason');
  }
  if (decision.action === 'approve-exception' && !(typeof decision.exceptionId === 'string' && decision.exceptionId.trim())) {
    throw new Error('map-gate --approve-exception requires an exception id');
  }
  if (decision.action === 'approve-exception' && !(typeof decision.reason === 'string' && decision.reason.trim())) {
    throw new Error('map-gate --approve-exception requires --reason with a human-readable reason');
  }
  if (decision.action === 'approve-exception' && !(typeof decision.scope === 'string' && decision.scope.trim())) {
    throw new Error('map-gate --approve-exception requires --scope <kind:id>');
  }
}

function runMapGate(runDir, decision, env = {}) {
  validateDecision(decision);
  const absRunDir = path.resolve(runDir);
  const reportDir = mapReportDir(absRunDir);
  const reportFile = path.join(reportDir, 'index.html');
  const snapshotFile = path.join(reportDir, 'report-snapshot.json');
  if (!fs.existsSync(reportFile) || !fs.existsSync(snapshotFile)) {
    throw new Error('map-gate requires current Report v0; rerun map-report');
  }

  const at = nowIso(env);
  const snapshot = readJson(snapshotFile);
  let freshness = applyReportArtifactFreshness(absRunDir, evaluateFreshness(absRunDir, snapshot), reportFile, snapshotFile);
  const inputs = readGateInputs(absRunDir, snapshot, freshness);
  const config = readGateConfig(absRunDir, snapshot, freshness);
  const inputBlockers = gateInputBlockers(inputs);
  const staticAssertions = inputs.staticAssertions || { assertions: [] };
  const motionAssertions = inputs.motionAssertions || { assertions: [] };
  let pageModel = inputs.pageModel || {};
  if (decision.action === 'approve-exception') {
    // Recording an exception writes page-model.json, which makes the on-disk
    // Report v0 stale. map-gate is a recorder, not a stage runner: it does not
    // regenerate the Report or fake its freshness. A later --approve stays
    // blocked until the human reruns map-report (CONTRACT.md Map Gate rules).
    pageModel = loadPageModel(absRunDir);
    pageModel = approveException(pageModel, decision, at);
    savePageModel(absRunDir, pageModel);
    freshness = applyReportArtifactFreshness(absRunDir, evaluateFreshness(absRunDir, snapshot), reportFile, snapshotFile);
  }
  const exceptionIds = approvedExceptionIds(pageModel);
  const approvedExceptions = new Set(exceptionIds);
  const approvedRecords = approvedExceptionRecords(pageModel);
  const waiverAudit = createWaiverAudit(approvedRecords);
  const discoveryBlockers = motionDiscoveryBlockers(inputs.motionCandidates, config, pageModel);
  const coverage = coverageSummary(inputs.staticCoverageText, pageModel, approvedRecords, discoveryBlockers, waiverAudit);
  const unknowns = unknownSummary(pageModel);
  const exceptions = exceptionBlockers(pageModel);
  const staticAssertionBlockers = requiredFailures(staticAssertions, 'static-map-assertions', approvedExceptions, pageModel, approvedRecords, waiverAudit);
  const motionAssertionBlockers = requiredFailures(motionAssertions, 'motion-scout-assertions', approvedExceptions, pageModel, approvedRecords, waiverAudit);
  const assertionBlockers = staticAssertionBlockers.concat(motionAssertionBlockers);
  const blockers = freshness.blockers
    .concat(inputBlockers)
    .concat(assertionBlockers)
    .concat(coverage.blockers)
    .concat(unknowns.blockers)
    .concat(exceptions);

  const gate = {
    schemaVersion: MAP_GATE_SCHEMA_VERSION,
    stage: 'map-gate',
    updatedAt: at,
    inputHashes: freshness.hashes,
    freshnessSummary: freshness.summary,
    assertionSummary: assertionSummary(staticAssertions, motionAssertions, assertionBlockers),
    coverageSummary: coverage.summary,
    unknownSummary: unknowns.summary,
    exceptionIds,
    exceptionWaivers: exceptionWaivers(waiverAudit),
    decision: decision.action === 'approve-exception'
      ? 'exception-approved'
      : (decision.action === 'reject' ? 'rejected' : (decision.action === 'approve' && !blockers.length ? 'approved' : 'blocked')),
    humanDecision: {
      action: decision.action,
      note: decision.note || null,
      reason: decision.reason || null,
      exceptionId: decision.exceptionId || null,
      scope: decision.scope || null,
    },
    blockers,
  };
  gate.artifacts = { gate: path.join(mapReportDir(absRunDir), 'gate.json') };
  writeGate(absRunDir, gate);

  if (decision.action === 'approve' && blockers.length) {
    const prefix = blockedApprovalPrefix(freshness);
    throw new MapGateBlockedError(`${prefix}: ${blockers.map(item => item.id).join(', ')}`, gate);
  }

  return gate;
}

module.exports = {
  MAP_GATE_SCHEMA_VERSION,
  runMapGate,
};
