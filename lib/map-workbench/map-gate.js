...
function assertionRegionId(row, pageModel) {
  ...
  return knownRegionIds(pageModel)
    ...
    .find(regionId => id.startsWith(`static-map-${regionId}-`) || id.startsWith(`motion-scout-${regionId}-`)) || null;
}
...
function assertionExcepted(row, pageModel, approvedExceptions, approvedExceptionRecords) {
  if (approvedExceptions.has(exceptionIdFor(row))) return true;
...
function assertionSummary(staticAssertions, motionAssertions, approvedExceptions = new Set(), pageModel = {}, approvedExceptionRecords = []) {
  ...
  const exceptedRequired = required
    .filter(row => row.status === 'fail' && assertionExcepted(row, pageModel, approvedExceptions, approvedExceptionRecords))
    .length;
...
function coverageExcepted(row, pageModel, approvedExceptionRecords) {
  ...
  return knownRegionIds(pageModel).includes(area) && approvedScopeMatchesRegion(area, approvedExceptionRecords);
}
...
function coverageSummary(staticCoverageText, motionCoverageText, pageModel = {}, approvedExceptionRecords = []) {
  const staticBlockers = coverageBlockers(staticCoverageText, 'static-map-coverage', pageModel, approvedExceptionRecords);
  const motionBlockers = coverageBlockers(motionCoverageText, 'motion-scout-coverage', pageModel, approvedExceptionRecords);
  return {
    summary: {
...
function runMapGate(runDir, decision, env = {}) {
  ...
  const coverage = coverageSummary(inputs.staticCoverageText, inputs.motionCoverageText, pageModel, approvedRecords);
